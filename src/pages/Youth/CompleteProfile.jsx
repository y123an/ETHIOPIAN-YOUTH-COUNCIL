...
const CompleteProfile = () => {
  ...
  const handleSubmit = async () => {
    ...
    const apiUrl = "youth/complete-profile";

    try {
      // Make an Axios POST request with data and a header
      const response = await Axios({
        method: "post",
        url: apiUrl,
        data: data,
        headers: {
          Authorization: `Bearer ${localStorage.getItem("youth-token")}`,
        },
      });
  ...
  useEffect(() => {
    const token = localStorage.getItem("youth-token");
    if (!token) {
      navigate("/");
    }
  }, []);
